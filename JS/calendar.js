...
const manipulate = () => {
    ...
    day.innerHTML = lit;
    reservedDates([3, 4, 5, 6, 7], lastdate, plusStart);
}

let reservedDates = (dates, lastdate, plusStart) => {
    // Loop to add the dates of the current month
    for (let i = 1; i <= lastdate; i++) {   
        for (let j = 0; j < dates.length; j++) { 
            let newDate = dates[j] + plusStart;
            if (i == newDate) {
                day.children[i].classList.add("reservado");
            }
        }
    }
}
